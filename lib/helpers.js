...
export function getDirectionalWarning(idea) {
  if (!idea) return null;
  const crtConflict = idea.crtBias && (
    (idea.direction === "long" && idea.crtBias === "bearish") ||
    (idea.direction === "short" && idea.crtBias === "bullish")
  );
  const insideDay = idea.dailyCandle === "inside_day";
  if (crtConflict || insideDay) {
    return { risk: "High Risk", action: "Do not trade" };
  }
  return null;
}
