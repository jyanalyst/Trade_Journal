...
export default function App() {
  const [ideas, setIdeas] = useState(() => {
    try { return JSON.parse(localStorage.getItem("sgx_ideas_v5")||"[]"); }
    catch { return []; }
  });
  ...
  useEffect(() => {
    try { localStorage.setItem("sgx_ideas_v5", JSON.stringify(ideas)); }
    catch {}
  }, [ideas]);
      ...
      <div style={{ maxWidth:500, margin:"0 auto" }}>
        <div style={{ display:"flex", justifyContent:"space-between",
          ...
          <div style={{ display:"flex", gap:6, alignItems:"center" }}>
            ...
            <button onClick={() => {
              if (window.confirm("Clear session and start fresh?")) setIdeas([]);
            }} style={{
